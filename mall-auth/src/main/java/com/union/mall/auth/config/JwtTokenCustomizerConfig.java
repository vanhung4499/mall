...
/**
 * JWT Custom Fields Configuration
 *
 * @author haoxr
 * @since 3.0.0
 */
        ...
        return context -> {
            if (OAuth2TokenType.ACCESS_TOKEN.equals(context.getTokenType()) && context.getPrincipal() instanceof UsernamePasswordAuthenticationToken) {
                ...
                Optional.ofNullable(context.getPrincipal().getPrincipal()).ifPresent(principal -> {
                    ...
                    if (principal instanceof SysUserDetails userDetails) { // System user adding custom fields
                        ...
                        // Store roles in JWT, roles parsed from JWT used for authorization: ResourceServerConfig#jwtAuthenticationConverter
                    ...
                    } else if (principal instanceof MemberDetails userDetails) { // Online mall member adding custom fields
